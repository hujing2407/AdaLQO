...
class summarizer:
    ...
    def loss_assign_center(self, x, y, new_radius=None, removed_centers=None, info=None, tem_id=None, loss=None):
        ...
        if max(noncluster_size) > 0:
            ...
            if random_cluster_id != -1:
                ...
                self.add_sample(x, y, True, -1,
                                new_radius=new_radius, removed_centers=removed_centers, info=info, tem_id=tem_id,
                                loss=loss)

        # else just ignore the query

    def loss_assign_noncenter(self, x, y, cluster_id, new_radius=None, removed_centers=None, info=None, tem_id=None,
                              loss=None):
    ...
    def rs_assign_noncenter(self, x, y, cluster_id, new_radius=None, removed_centers=None, info=None, tem_id=None,
                            loss=None):
        # find out max clusters
        if self.is_print:
    ...
    def ODMedoids(self, x, y):
        # Return 4 values:
        # 1. If increase the buffer size by 1 (always True in ODMedoids)
